...
class OnlineRepository : LogRepositoryBase {
  ...
  public:
    OnlineRepository(const std::string &username, const std::string &password,
                     const std::string &apiBaseUrl, const std::string &apiPort)
        : m_apiBaseUrl{apiBaseUrl}, m_apiPort{apiPort} {
        auth();
    }

    bool isLoggedIn() { return not m_sessionId.empty(); }
