...
const static auto SECTION_TITLE_REGEX =
    std::regex { "^# \\s*(.*?)\\s*$" };  // section title group group 1
...
YearLogEntryData DefaultLogRepository::collectDataForYear(unsigned year) {
    ...
    for (unsigned month = Date::JANUARY; month <= Date::DECEMBER; month++) {
...
std::string DefaultLogRepository::getLogEntryPath(const Date& date) {
    return dateToLogFilePath(date);
}

std::string DefaultLogRepository::readFile(const Date& date) {
    std::ifstream t(getLogEntryPath(date));
...
void DefaultLogRepository::removeFile(const Date& date) {
    std::remove(getLogEntryPath(date).c_str());
}
