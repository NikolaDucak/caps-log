...
public:
    ...
    bool handleInputEvent(const UIEvent& event) override {
        switch (event.type) {
            case UIEvent::ROOT_EVENT: return handleRootEvent(event.input); 
            case UIEvent::FOCUSED_DATE_CHANGE: handleFocusedDateChange(event.input); break; 
            case UIEvent::FOCUSED_TAG_CHANGE: handleFocusedTagChange(event.input); break; 
            case UIEvent::FOCUSED_SECTION_CHANGE: handleFocusedSectionChange(event.input); break; 
            case UIEvent::CALENDAR_BUTTON_CLICK: handleCalendarButtonClick(); break;
        };
        return true;
    };
...
private:
    ...
    bool handleRootEvent(const int input) {
        switch (input) {
            case '\x1B': quit(); break;
            case 'd': deleteFocusedLog(); break;
            case '+': displayYear(+1); break;
            case '-': displayYear(-1); break;
            case 'm': toggle(); break;
...
private:
    static const YearMap<bool>* findOrNull(const StringYearMap& map, std::string key) {
        ...
        if (result == map.end()) { return nullptr; }
    ...
    static std::vector<std::string> makeMenuTitles(const StringYearMap& map) {
        ...
        strs.push_back(" ----- ");
        for (auto const& imap: map)
            strs.push_back(std::string("(") + std::to_string(imap.second.daysSet()) + ") - " + imap.first);
